...
static VALUE xmp_multiple_value_to_ruby(const Exiv2::Value& value) {
  VALUE values = rb_ary_new2(value.count());
  for(int i = 0; i < value.count(); i++) {
    rb_ary_store(values, i, to_ruby_string(value.toString(i)));
  }
  return values;
}
...
static VALUE value_to_ruby(const Exiv2::Value& value) {
  ...
  switch(typeId) {
    ...
    case Exiv2::xmpBag:
    case Exiv2::xmpSeq: {
      return xmp_multiple_value_to_ruby(value);
    }
    case Exiv2::langAlt: {
      VALUE langAlt = rb_hash_new();

      const Exiv2::LangAltValue* lav = dynamic_cast<const Exiv2::LangAltValue*>(&value);
      if(!lav) {
        return Qnil;
      }

      std::map<std::string, std::string>::const_iterator i = lav->value_.begin();
      for( ; i != lav->value_.end(); ++i )
      {
        rb_hash_aset(langAlt, to_ruby_string(i->first), to_ruby_string(i->second));
      }
      return langAlt;
    }
  }
  ...
  switch(typeId) {
    ...
    case Exiv2::xmpSeq: {
      ...
      for(i = 0; i < RARRAY_LEN(ary); i++) {
        container->read(to_std_string(rb_ary_entry(ary, i)));
      }
      return;
    }
    case Exiv2::date: {
...
extern "C" {
  ...
  static VALUE exif_data_add(VALUE self, VALUE key, VALUE value) {
    ...
    for (i = 0; i < RARRAY_LEN(ary); i++) {
      ...
      ruby_to_value(v.get(), rb_ary_entry(ary, i));
  ...
  static VALUE iptc_data_add(VALUE self, VALUE key, VALUE value) {
    ...
    if(repeatable) {
      ...
      for (i = 0; i < RARRAY_LEN(ary); i++) {
        ...
        if(v.get() && v->ok()) {
          data->add(iptcKey, v.get());
        }
      }
      return Qtrue;
    } else {
      Exiv2::Value::AutoPtr v = Exiv2::Value::create(typeId);
      ruby_to_value(v.get(), value);

      if(v.get() && v->ok() && !data->add(iptcKey, v.get())) {
        return Qtrue;
      }
    }
  ...
  static VALUE xmp_data_add(VALUE self, VALUE key, VALUE value) {
    ...
    if(v.get() && v->ok() && !data->add(xmpKey, v.get())) {
      return Qtrue;
    }
    return Qfalse;
  }
